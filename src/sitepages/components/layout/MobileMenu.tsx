...
import { X, ArrowRight, Home, LayoutGrid, Users, PhoneCall, ChevronRight, HelpCircle } from "lucide-react";
...
export default function MobileMenu({ isOpen, onClose }: MobileMenuProps) {
    ...
    return (
        <>
            {/* Backdrop Overlay */}
            <div
                className={`fixed inset-0 z-50 bg-[#07010f]/80 backdrop-blur-md transition-opacity duration-300 ${isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
                    }`}
                onClick={onClose}
            />
            ...
            {/* Drawer Container */}
            <div
                className={`fixed top-0 right-0 bottom-0 z-50 w-full max-w-[320px] bg-[#140620] border-l border-purple-500/20 shadow-[0_0_50px_rgba(139,92,246,0.15)] transition-transform duration-300 ease-out flex flex-col ${isOpen ? "translate-x-0" : "translate-x-full"
                    }`}
            >
                ...
                <div className="relative z-10 flex items-center justify-between px-6 py-5 border-b border-purple-500/10">
                    <Link href="/" className="flex items-center text-white text-xl font-bold tracking-tighter" onClick={onClose}>
                        ...
                        <span className="ml-2 font-black italic">enteropia</span>
                    </Link>
                    <button
                        onClick={onClose}
                ...
                {/* Body Links */}
                <div className="relative z-10 flex-1 overflow-y-auto px-6 py-8 flex flex-col gap-6">
                    ...
                    <nav className="flex flex-col gap-2">
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <span className="font-semibold text-sm">Tech Partners</span>
                            </div>
                            <ChevronRight className="w-4 h-4 opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all text-purple-400" />
                        </Link>
                        ...
                        <div
                            className="group flex items-center justify-between p-3 rounded-xl border border-transparent hover:border-purple-500/10 hover:bg-purple-900/10 transition-all text-white/90 hover:text-white cursor-pointer"
                        >
                            <div className="flex items-center gap-3">
                                <div className="p-2 rounded-lg bg-purple-900/20 text-purple-400 group-hover:bg-purple-800/40 group-hover:text-purple-300 transition-colors">
