...
interface ServicesClientProps {
  services: Service[];
  paramServicesName: string; // The dynamic folder parameter (usually "services")
}
...
export default function ServicesClient({ services, paramServicesName }: ServicesClientProps) {
  const [activeCategory, setActiveCategory] = useState("all");
  const [openFaq, setOpenFaq] = useState<number | null>(null);
  const [expandedSubs, setExpandedSubs] = useState<{ [serviceId: string]: string | null }>({});

  // Initialize expanded subservices
  useEffect(() => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`lg:col-span-5 space-y-6 ${isEven ? "lg:order-1" : "lg:order-2"}`}>
                    ...
                    <div className="pt-2">
                      <Link
                        href={`/${paramServicesName}/${service.slug}`}
