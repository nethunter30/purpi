...
import {
  ...
  ShieldAlert, FolderTree, X, Eye, EyeOff, Upload,
} from "lucide-react";
...
export default function CategoriesPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    uploadData.append("subfolder", "categories");
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      else setError(result.message || "Failed to fetch categories");
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    const url = editingId ? `/api/services/categories/${editingId}` : "/api/services/categories";
    ...
    try {
      ...
      const result = await res.json();
      if (result.success) { fetchCategories(); closeModal(); }
      else setError(result.message || "Failed to save category");
    } catch {
  ...
  const toggleActive = async (cat: ICategory) => {
    try {
      const res = await fetch(`/api/services/categories/${cat._id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h2 className="text-lg font-bold text-white flex items-center gap-2">
          <FolderTree className="w-5 h-5 text-[#a356db]" />
          Categories
      ...
      <div className="bg-[#0f0418]/60 rounded-xl border border-purple-900/20 overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-gray-500">
            ...
            <p className="text-sm font-light">No categories yet. Click "Add Category" to start.</p>
          </div>
        ) : (
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-purple-900/10">
              {filtered.map((cat) => (
                <tr key={cat._id} className="hover:bg-purple-900/5 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400 font-light line-clamp-1">{cat.description}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    ...
                    >
                      {cat.isActive ? <><Eye className="w-3 h-3" /> Active</> : <><EyeOff className="w-3 h-3" /> Hidden</>}
                    </button>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => openEdit(cat)} className="p-1.5 text-blue-400 hover:text-blue-300 hover:bg-blue-400/10 rounded-lg transition-colors cursor-pointer" title="Edit">
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete(cat._id, cat.name)} className="p-1.5 text-red-400 hover:text-red-300 hover:bg-red-400/10 rounded-lg transition-colors cursor-pointer" title="Delete">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-white text-xs font-medium">Category Image</label>
